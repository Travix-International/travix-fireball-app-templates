...
if [ -z "$APPIX_BIN_URL" ]; then
    if [ -z "$APPIX_VERSION" ]; then
        ...
        APPIX_BIN_URL="https://raw.githubusercontent.com/Travix-International/travix-fireball-app-templates/master/appixBinariesPoc/$APPIX_BIN_NAME"
    else
        # Get specific version
        # TODO: the file naming scheme needs to be finalized once we figure out how we build and publish the binaries
        APPIX_BIN_URL="https://raw.githubusercontent.com/Travix-International/travix-fireball-app-templates/master/appixBinariesPoc/$APPIX_BIN_NAME-$APPIX_VERSION"
    fi
fi
